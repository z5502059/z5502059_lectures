...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for tic in tickers:
        ...
        df_tic = df_tic[[prc_col]].copy()
        df_tic.columns = [f"{tic.lower()}"]
        df_raw = pd.concat([df_raw, df_tic], axis=1)
...
def mk_ret_df(prc_df):
    ...
    """
    market_data = pd.read_csv(cfg.FF_CSV, parse_dates=['Date'], index_col='Date')
    market_daily_returns = market_data[['mkt']]
    market_daily_returns.sort_index(inplace=True)
    prc_r = prc_df.pct_change()
    df = prc_r.join(market_daily_returns, how='left')
    return df
...
def mk_aret_df(ret_df):
    ...
    """
    data_columns = ret_df.iloc[:, :-1].columns
    for col in data_columns:
        ret_df[col] -= ret_df['mkt']
...
def get_avg(df, col, year):
    ...
    """
    df.sort_index(inplace=True)
    col_values = df.loc[str(year), col]
    avg_value = col_values.mean(axis=0, skipna=True)
    return avg_value
...
def get_ew_rets(df, tickers):
    ...
    """
    df.sort_index(inplace=True)
    df_selected = df.loc[:, tickers]
    df_selected.dropna(axis=0, how='all', inplace=True)
    ew_returns = df_selected.mean(axis=1, skipna=True)
    return ew_returns
...
def get_ann_ret(ser, start, end):
    ...
    no_missing_ser = ser.loc[start:end].dropna()
    selected_values = no_missing_ser.add(1, fill_value=None)
    N = selected_values.count()
    ann_return = selected_values.prod() ** (252 / N) - 1
    return ann_return
